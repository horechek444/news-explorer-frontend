...
const NewsCardList = ({articles, getNewsError, loading, loggedIn, onRegisterPopupOpen}) => {
  ...
  return (
    <section className={handleShowResults()}>
      ...
      <ul className="news-cards__list">
        {articles.slice(0, count).map((article, index) => <NewsCard key={index} article={article} loggedIn={loggedIn} onRegisterPopupOpen={onRegisterPopupOpen}/>)}
      </ul>
      <button
