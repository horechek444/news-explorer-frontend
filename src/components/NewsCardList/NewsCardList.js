...
const NewsCardList = ({articles, getNewsError, loading, loggedIn}) => {
  ...
  return (
    <section className={handleShowResults()}>
      ...
      <ul className="news-cards__list">
        {articles.map((article, index) => <NewsCard key={index} article={article} index={index} count={count} loggedIn={loggedIn}/>)}
      </ul>
      <button
